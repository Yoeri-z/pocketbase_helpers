...
class MockSubscription {
  void Function(RecordSubscriptionEvent)? _callback;
...
void main() {
  ...
  group('watchOne', () {
    test('emits record updates', () async {
      ...
      final stream = helper.watchOne(id: actual.id);

      final expectFuture = expectLater(
        stream,
        emits(predicate<DummyRecord?>((record) => record!.equals(actual))),
      );
      ...
      await expectFuture;
      verify(() => service.subscribe(actual.id, any())).called(1);
    });
  });
  ...
  group('watchList', () {
    test('triggers re-fetch on any collection change', () async {
      ...
      expect(events.length, equals(1));
      expect(events.first.items.length, equals(1));
      expect(events.first.items.first.equals(items[0].$2), isTrue);
  ...
  group('watchOneOrNull', () {
    test('triggers re-fetch on any collection change', () async {
      ...
      ).thenAnswer((_) async => ResultList(items: [model]));
      ...
      expect(events.length, equals(1));
      expect(events.first!.equals(actual), isTrue);
